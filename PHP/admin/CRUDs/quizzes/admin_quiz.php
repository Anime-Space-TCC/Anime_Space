...
<body class="admin-cruds">
  <div class="admin-links">
    ...
    <form method="GET" class="admin-busca">
      <input type="text" name="buscarQuiz" placeholder="Buscar quiz..."
        value="<?= htmlspecialchars($_GET['buscarQuiz'] ?? '') ?>">
      ...
      <tbody>
        <?php foreach ($quizzes as $q): ?>
          <tr>
            <td>
              <?php if (!empty($q['capa'])): ?>
                <img src="../../../../img/<?= htmlspecialchars($q['capa']) ?>" alt="<?= htmlspecialchars($q['titulo']) ?>"
                  width="100">
              <?php else: ?>
                <span>Sem capa</span>
              <?php endif; ?>
            ...
            <td>
              ...
              <a href="../../../../PHP/admin/CRUDs/quizzes/perguntas.php?quiz_id=<?= $q['id'] ?>"
                class="admin-btn">Perguntas</a>
              <a href="../../../../PHP/admin/CRUDs/quizzes/quiz_delete.php?id=<?= $q['id'] ?>" class="admin-btn"
                onclick="return confirm('Excluir este quiz?')">Excluir</a>
            </td>
          </tr>
        <?php endforeach; ?>
      </tbody>
