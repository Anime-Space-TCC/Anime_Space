  ...
  fetch("../../PHP/shared/perfil_data.php", { cache: "no-store" })
    ...
    .then((data) => {
      ...
      const favContainer = document.querySelector(
        ".favoritos-section .cards-container"
      );
      if (favContainer) {
      ...
      const histContainer = document.querySelector(
        ".historico-section .cards-container"
      );
      if (histContainer) {
        ...
        if (data.historico.length) {
          data.historico.forEach((h) => {
            ...
            card.className = "card";
            card.innerHTML = `<img src="../../img/${h.miniatura}" alt="${
              h.titulo
            }">
                                          <p>${h.titulo}</p>
                                          <small>${new Date(
      ...
      // Recomendações
      const recContainer = document.querySelector(
        ".recomendacoes-section .cards-container"
      );
      if (recContainer) {
      ...
      if (levelElem)
        levelElem.innerHTML = `Nível: ${data.nivel}<p class="titulo">${data.tituloNivel}</p>`;
